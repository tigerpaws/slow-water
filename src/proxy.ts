...
// one fixed password, sent as a header over HTTPS, for a short-lived demo.
const PASSWORD = process.env.SITE_PASSWORD ?? "this-is-fine-1984";

export function proxy(request: NextRequest) {
  // Local dev stays open; the gate exists for the deployed site.
  if (process.env.NODE_ENV === "development") return NextResponse.next();
  if (request.headers.get("x-site-password") === PASSWORD) return NextResponse.next();
